(function ($) {
	...
	this.Topbar = Backbone.View.extend({
			...
			if (window.nodewebkit) {
				...
				if (!!$favicon.data('on') !== !!notificationCount) {
					if (notificationCount) {
						$favicon.attr('href', Config.routes.root + '/favicon-notify.ico');
						$favicon.data('on', '1');
					} else {
						$favicon.attr('href', Config.routes.root + '/favicon.ico');
						$favicon.data('on', '');
					}
				}
			}
		...
		updateTabbarMini: function () {
			...
			var buf = '<ul><li><a class="button minilogo' + notificationClass + '" href="' + app.root + '"><img src="' + Config.routes.root + 'favicon-256.png" width="32" height="32" alt="Pok&eacute;mon Showdown! (beta)" /><i class="fa fa-caret-down" style="display:inline-block"></i></a></li></ul>';
